...
class DailySchedule < ApplicationRecord
  ...
  def self.update_schedule
    ...
    response = open('https://api.sportradar.us/mlb-t6/en/games/' + current_year.to_s + '/' + current_month.to_s + '/' + current_day.to_s + '/schedule.json?api_key=' + ENV['SPORTRADAR_KEY']).read
